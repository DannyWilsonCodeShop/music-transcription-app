...
def detect_bass_notes(bass_audio: np.ndarray, sr: int, tempo: float, 
    ...
    for measure in measures[:4]:
        log.info(f"  Measure {measure['measure']}: | {measure['nns_display']} | ({measure['notes_display']})")
...
def quantize_notes(notes: List[Dict], tempo: float, time_signature: str, 
    ...
    """
    Quantize note ATTACKS to 16th note grid
    
    Key concept: We only care about when NEW notes are played (attacks),
    not sustained notes. This detects note onsets based on:
    1. Pitch change
    2. Volume increase (attack)
    3. Silence before the note
    """
    beats_per_measure = int(time_signature.split('/')[0])
    beat_duration = 60.0 / tempo
    sixteenth_duration = beat_duration / 4
    
    # First, detect note attacks (new notes being played)
    note_attacks = []
    for i, note in enumerate(notes):
        ...
        if i == 0:
            ...
            # Check if this is a new note attack:
            # 1. Pitch changed (different note)
            if note['pitch'] != prev_note['pitch']:
                is_attack = True
            
            # 2. Gap in time (silence before this note)
            elif note['start'] - prev_note['end'] > 0.05:  # 50ms gap
                is_attack = True
            
            # 3. Volume increase (re-attack of same note)
            elif note.get('velocity', 0) > prev_note.get('velocity', 0) * 1.3:
                is_attack = True
        
        if is_attack:
            note_attacks.append(note)
    
    log.info(f"  Detected {len(note_attacks)} note attacks from {len(notes)} total notes")
    
    # Now quantize only the attacks to 16th note grid
    quantized = []
    for note in note_attacks:
...
def group_by_measures(notes: List[Dict], tempo: float, time_signature: str,
    ...
    """
    Group notes by measure with beat-level placeholders for readability
    
    Format: | 1 - - - | for whole note in 4/4
            | 1 - 5 - | for half notes on beats 1 and 3
            | 1 4 5 1 | for quarter notes on each beat
    
    Placeholders (-) show beats without new note attacks for easier reading.
    """
    beats_per_measure = int(time_signature.split('/')[0])
    beat_duration = 60.0 / tempo
    ...
    for measure_num in range(1, total_measures + 1):
        ...
        # Get note ATTACKS in this measure
        measure_notes = [n for n in notes if measure_start <= n['quantized_start'] < measure_end]
        
        if len(measure_notes) == 0:
            continue
        
        # Create beat grid with placeholders
        beat_grid = []  # List of (beat_number, nns, note_name) or (beat_number, '-', '')
        
        for beat_num in range(1, beats_per_measure + 1):
            beat_start = measure_start + ((beat_num - 1) * beat_duration)
            beat_end = beat_start + beat_duration
            
            # Find notes that start on this beat (within tolerance)
            beat_tolerance = beat_duration * 0.25  # 25% of beat duration
            notes_on_beat = [
                n for n in measure_notes 
                if abs(n['quantized_start'] - beat_start) < beat_tolerance
            ]
            
            if notes_on_beat:
                # Use the first note if multiple (shouldn't happen with good quantization)
                note = notes_on_beat[0]
                beat_grid.append({
                    'beat': beat_num,
                    'nns': note['nns'],
                ...
                # Placeholder for beat without new note
                beat_grid.append({
                    'beat': beat_num,
                    'nns': '-',
                    'note_name': '',
        ...
        nns_display = ' '.join([b['nns'] for b in beat_grid])
        notes_display = ' '.join([b['note_name'] if b['has_note'] else '-' for b in beat_grid])
        ...
        measures.append({
            ...
            'end': measure_end,
            'notes': measure_notes,
            'beat_grid': beat_grid,  # Beat-by-beat breakdown
            'nns_display': nns_display,  # "1 - - -" or "1 - 5 -"
            'notes_display': notes_display,  # "Bb - - -" or "Bb - F -"
            'nns': [n['nns'] for n in measure_notes],  # Just the attacks
            'noteNames': [n['note_name'] for n in measure_notes],
            'pitches': [n['pitch'] for n in measure_notes],
            'attack_count': len(measure_notes)
        })
