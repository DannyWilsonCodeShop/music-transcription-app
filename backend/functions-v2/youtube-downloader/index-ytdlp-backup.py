...
def download_with_ytdlp(youtube_url, job_id):
    ...
    # Try to download cookies from S3 if available
    try:
        cookies_bucket = os.environ.get('COOKIES_BUCKET', AUDIO_BUCKET)
        cookies_key = os.environ.get('COOKIES_KEY', 'cookies/youtube-cookies.txt')
        logger.info(f"Attempting to download cookies from s3://{cookies_bucket}/{cookies_key}")
        s3_client.download_file(cookies_bucket, cookies_key, cookies_path)
        logger.info("Cookies downloaded successfully")
    except Exception as e:
        logger.warning(f"Could not download cookies (will try without): {str(e)}")
        cookies_path = None
